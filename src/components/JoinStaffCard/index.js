...
export const JoinStaffCard = ({
  ...
  const AdminLabel = () => {
    return <div className="admin-label"> Admin </div>;
  };

  const tinyLinkSpacing = !(tickets && tenants)
    ? 'tiny-link'
    : 'tiny-link-info';
    ...
    <>
      <div className="card card-length">
        ...
        <div className="card-content">
          {admin && <AdminLabel />}
          <p className="info-font">{phoneNumber}</p>
          <p className="info-font">{email}</p>
          <div className="empty-space"></div>
          <p className="tiny-font">Set out of office</p>
          <hr className="card-divider"></hr>
          {tickets && <InfoLabel type="tickets" />}
          {tenants && <InfoLabel type="tenants" />}
          <a href="#" className={tinyLinkSpacing}>
            {' '}
            Reassign{' '}
          </a>
        </div>
