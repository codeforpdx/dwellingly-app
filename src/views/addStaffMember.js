...
export const InfoField = ({ label, info }) => {
  const infoField =
    'has-text-weight-bold placeholder ' +
    (label === 'Phone'
      ? 'phone-field'
      : label === 'Email'
      ? 'email-field'
  ...
  return (
    <div>
      ...
      <span className="input-field">
        {' '}
        {label}
        <p className={infoField} contentEditable required></p>
      </span>
    </div>
  );
};
      ...
      <div className="add-staff-page-spacing">
        ...
        <div className="add-staff-page-length">
          ...
          <div className="save-button-spacing">
            <button className="save-new-staff-button" onClick={handleSave}>
              {' '}
              SAVE{' '}
            </button>
          </div>
          <button
            className="button has-background-grey has-text-white is-rounded is-small has-text-weight-bold cancel-new-staff"
