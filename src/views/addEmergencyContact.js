...
const makeAuthHeaders = ({ user }) => ({
  headers: { Authorization: `Bearer ${user.accessJwt}` },
});
...
const validationSchema = Yup.object().shape({
  ...
  description: Yup.string().max(
    256,
    "*Description can't be longer than 256 characters"
  ),
  contact_numbers: Yup.array().of(
    Yup.object()
      .shape({
        ...
        numtype: Yup.string().max(
          30,
          "*Number type can't be longer than 30 characters"
        ),
        extension: Yup.string().max(
          10,
          "*Extension can't be longer than 10 digits"
        ),
      })
...
const NumberSubForm = ({ i, values, errors, handleChange }) => {
  const subFormErrors =
    errors.contact_numbers && errors.contact_numbers[i]
      ? errors.contact_numbers[i]
      : null;
  return (
    <>
      <div className="form-row columns">
        <label
          ...
          htmlFor={`contact_numbers[${i}].number`}>
          Phone Number
        </label>
        <Field
      ...
      <div className="form-row columns">
        <label
          className="column is-one-quarter"
          htmlFor={`contact_numbers[${i}].numtype`}>
          Phone Number Type
        </label>
        <Field
          className="column form-field"
      ...
      <div className="form-row columns">
        <label
          className="column is-one-quarter"
          htmlFor={`contact_numbers[${i}].extension`}>
          Extension
        </label>
        <Field
          className="column form-field"
...
const AddEmergencyContact = (props) => {
  ...
  const formHandler = (data) => {
    const startPost = () =>
      axios.post(`/api/emergencycontacts`, data, makeAuthHeaders(userContext));
    const startPut = () =>
      axios.put(
        `/api/emergencycontacts/${data.id}`,
        data,
  ...
  return (
    <div className="add-emergency-contact__container">
      <h2 className="page-title">
        {editMode ? 'Edit' : 'Create'} Emergency Contact
      </h2>
      {loading && (
        ...
        <Formik
          ...
          {({
            ...
            <div className="form-container add-emergency-contact__main_container">
              <Form
                className="add-emergency-contact__form-container"
                onSubmit={handleSubmit}>
                <div className="form-row columns">
                  <label className="column is-one-quarter" htmlFor="name">
                    Contact Name
                  </label>
                  <Field
                    className="column form-field"
                ...
                <FieldArray
                  ...
                  render={(numbersArrayFields) => {
                    const addRowValid =
                      values.contact_numbers[0].number !== '' &&
                      ...
                      <>
                        ...
                        <div className="form-add-rows-container">
                          <span
                            className={`${
                              addRowValid ? 'active' : ''
                            } is-rounded form-add-rows`}
                            onClick={addRow}>
                            <i className="fas fa-plus-circle"></i> Add New Phone
                            Number
                          </span>
                        </div>
                      </>
