...
const userAPICall = (context, method, updateUserInfo, data) => {
  ...
  isPatchCall
    ...
    : axios[method](`http://localhost:5000/api/user/${context.user.identity}`, {
        ...
        .catch((error) => {
          alert(
            `I'm sorry! There was an error getting your information from the database. Please try again later: ${error}`
          );
        });
};
...
const Settings = () => {
  ...
  return (
    <UserContext.Consumer>
      {(context) => {
        {
          /* change this to useEffect to make testing work.... */
        }
        if (userFormInfo.email === "") {
          getUser(context, updateUserInfo);
        }

        return userFormInfo.email !== "" && (
